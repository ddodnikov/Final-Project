		...
		if (UserDAO.getUserDAOInstance().isEmailUsed(email)) {
			...
			if (password1.length() < MIN_PASSWORD_LENGTH) {
				...
				if (!password1.equals(password2)) {
					...
					String main = "D:\\soundcloudFiles\\" + email;
					String images = main + "\\images";
					String tracks = main + "\\tracks";
					
					File mainUserDirectory = new File(main);
					if (!mainUserDirectory.exists()) {
						mainUserDirectory.mkdir();
					}
					File userImagesDirectory = new File(images);
					if (!userImagesDirectory.exists()) {
						userImagesDirectory.mkdir();
					}
					File userTracksDirectory = new File(tracks);
					if (!userTracksDirectory.exists()) {
						userTracksDirectory.mkdir();
					}
