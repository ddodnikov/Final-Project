...
public class ShowProfile extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (request.getSession(false) == null || request.getSession(false).getAttribute("currentUser") == null) {
			...
			List<Playlist> showPlaylists = PlaylistDAO.getPlaylistDAOInstance().getUserPlaylists(user_id, 0);
			request.getSession().setAttribute("showPlaylists", showPlaylists);
			request.getSession().setAttribute("userPlaylistsShown", 0);
