...
public class UserDAO extends AbstractDAO implements IUserDAO {
	...
	public static final String UPDATE_USER_INFO = "UPDATE users SET display_name = ?, first_name = ?, last_name = ?, "
			+ "city = ?, country = ?, biography = ? WHERE user_id = ?;";
	...
	public User getUserById(int id) {
		...
		try {
			ps = getCon().prepareStatement(SELECT_USER_BY_ID);
			ps.setInt(1, id);
			...
			if (rs.next()) {
				...
				result.setUserImageID(rs.getInt(10));
				result.setHeaderImageID(rs.getInt(11));
			}
		} catch (SQLException e) {
			e.printStackTrace();
		}		
	...
	public User selectUserByTrackId(int track_id) {
		try {
			...
			if (rs.next()) {
				...
				user.setUserImageID(rs.getInt("user_img_id"));
				user.setHeaderImageID(rs.getInt("header_img_id"));
			}
			return user;
		} catch (SQLException e) {
			e.printStackTrace();
	...
	public void updateUserInfo(int userId, String displayName, String firstName, String lastName, 
		...
		PreparedStatement ps;
			...
			try {
				ps = getCon().prepareStatement(UPDATE_USER_INFO);
